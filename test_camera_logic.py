...
test_events = [
    ...
    # Движение мыши с зажатой ПКМ (движение камеры) - теперь относительные дельты
    ('mouse_move_relative', (10, 5, 0.6)),   # +10 вправо, +5 вниз
    ('mouse_move_relative', (10, 10, 0.7)),  # +10 вправо, +10 вниз
    ('mouse_move_relative', (5, 10, 0.8)),   # +5 вправо, +10 вниз
    ('mouse_move_relative', (5, 10, 0.9)),   # +5 вправо, +10 вниз
...
def simulate_camera_logic():
    ...
    for event in test_events:
        ...
        if event_type == 'mouse_pos':
            ...
            dx, dy = event_args[0], event_args[1]
            if 'right' in pressed_buttons and rmb_center is not None:
                if last_mouse_pos is None:
                    last_mouse_pos = rmb_center
                next_pos = (last_mouse_pos[0] + dx, last_mouse_pos[1] + dy)
                print(f"📹 Относительное движение камеры: Δ({dx}, {dy})")
                print(f"   - Текущая позиция до применения: {last_mouse_pos}")
                print(f"   - Ожидаемая новая позиция (виртуальная): {next_pos}")
                print(f"   - Отправляем относительное движение: send_relative_line({dx}, {dy})")
                last_mouse_pos = next_pos
            else:
                print(f"⚠️ Получено относительное движение без зажатой ПКМ: Δ({dx}, {dy})")
